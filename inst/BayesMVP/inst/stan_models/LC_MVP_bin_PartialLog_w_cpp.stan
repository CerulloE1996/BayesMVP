...
//       }
      ...
      matrix Phi_approx_binary_Z_and_log_lik_stable(vector Bound_Z, vector y_vec, vector u_vec) {
            ...
            vector[n_obs] log_Phi_signed = mvp_log_inv_logit(poly_signed);
            vector[n_obs] log_1m_Phi_signed = mvp_log_inv_logit(-poly_signed);
            ...
            matrix[n_obs, 2] Z_and_log_lik;
            Z_and_log_lik[, 1] = mvp_inv_Phi_approx_from_logit_prob(sign_y .* (log_sum_exp_2d(tmp_array_2d_to_lse) - (log_1m_v + log_1m_Phi_signed)));
      ...
      matrix Phi_approx_interval_Z_and_log_lik_stable(vector Bound_Z_lo, vector Bound_Z_hi, vector u_vec) {
            ...
            vector[n_obs] log_Phi_lo = mvp_log_inv_logit(poly_lo);
            vector[n_obs] log_Phi_hi = mvp_log_inv_logit(poly_hi);
            vector[n_obs] log_1m_Phi_lo = mvp_log_inv_logit(-poly_lo);
            vector[n_obs] log_1m_Phi_hi = mvp_log_inv_logit(-poly_hi);
            ...
            matrix[n_obs, 2] Z_and_log_lik;
            Z_and_log_lik[, 1] = mvp_inv_Phi_approx_from_logit_prob(log_sum_exp_2d(tmp_lower) - log_sum_exp_2d(tmp_upper));
                                  ...
                                  for (t in 1 : n_tests) {
                                      ...
                                      real Bound_Z =  -(Xbeta_n[c, t] + inc) * L_Omega_diag_recip[c, t];
                                             ...
                                             if (Phi_type == 2) {
                                                  ...
                                                  real log_Phi_Bound_Z = mvp_log_inv_logit(poly_Bound_Z);
                                                  real log_1m_Phi_Bound_Z = mvp_log_inv_logit(-poly_Bound_Z);
                                                  if (y[n, t] == 1) {
                                                    //// q = Phi + (1 - Phi) u,  1 - q = (1 - Phi)(1 - u)
                                                    Z_std_norm[t] = mvp_inv_Phi_approx_from_logit_prob(log_sum_exp(log_Phi_Bound_Z, log_1m_Phi_Bound_Z + log(u[t])) - (log1m(u[t]) + log_1m_Phi_Bound_Z));
                                                    y1[t] = log_1m_Phi_Bound_Z;
                                                  } else {
                                                    //// q = Phi u,  1 - q = (1 - Phi) + Phi (1 - u)
                                                    Z_std_norm[t] = mvp_inv_Phi_approx_from_logit_prob((log(u[t]) + log_Phi_Bound_Z) - log_sum_exp(log_1m_Phi_Bound_Z, log_Phi_Bound_Z + log1m(u[t])));
