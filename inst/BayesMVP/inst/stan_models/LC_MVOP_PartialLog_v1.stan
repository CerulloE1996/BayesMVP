                        ...
                        for (c in 1:n_class) {
                            ...
                            for (t in 1:n_tests) {
                                ...
                                if (t <= n_binary_tests) {
                                    ...
                                    if (k_obs == 1) {
                                        Phi_lower = 0.0;
                                        Phi_upper = Phi((C_t[1] - mu_plus_inc) * L_tt_recip);
                                    } else if (k_obs == n_cat_t) {
                                        Phi_lower = Phi((C_t[n_thr_t] - mu_plus_inc) * L_tt_recip);
                                        Phi_upper = 1.0;
                                    } else {
                                        Phi_lower = Phi((C_t[k_obs - 1] - mu_plus_inc) * L_tt_recip);
                                        Phi_upper = Phi((C_t[k_obs] - mu_plus_inc) * L_tt_recip);
                                    }
