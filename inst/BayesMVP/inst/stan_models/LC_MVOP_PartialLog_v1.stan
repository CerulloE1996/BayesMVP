...
transformed parameters {
      ...
      if (prior_only == 0) {
                ...
                // ---- Likelihood with log_sum_exp_2d:
                {
                    matrix[N, n_class] lp;
                    
                    for (n in 1:N) {
                        
                        vector[n_tests] u = lb_ub_jacobian(to_vector(u_raw[n,]), 0.0, 1.0);
                        ...
                        for (c in 1:n_class) {
                            ...
                            for (t in 1:n_tests) {
                                ...
                                if (t <= n_binary_tests) {
                                    ...
                                    vector[2] Z_and_log_lik;
                                    if (k_obs == 1) {
                                        Z_and_log_lik = Phi_exact_binary_step((C_t[1] - mu_plus_inc) * L_tt_recip, 0.0, u[t]);
                                    } else if (k_obs == n_cat_t) {
                                        Z_and_log_lik = Phi_exact_binary_step((C_t[n_thr_t] - mu_plus_inc) * L_tt_recip, 1.0, u[t]);
                                    } else {
                                        Z_and_log_lik = Phi_exact_interval_step((C_t[k_obs - 1] - mu_plus_inc) * L_tt_recip,
                                                                                (C_t[k_obs] - mu_plus_inc) * L_tt_recip,
                                                                                u[t]);
                                    }
                                    Z_std_norm[t] = Z_and_log_lik[1];
                                    y1[t] = Z_and_log_lik[2];
