...
transformed parameters { 
      ...
      ////
      //// t = ORIGINAL test index; s = fitted slot holding it (all test-indexed DATA is in fitted layout):
      ////
      for (t in 1:n_tests) {
          int s = inv_perm[t];
...
generated quantities {
      ...
      ////
      vector[n_tests] Se_baseline;
      vector[n_tests] Sp_baseline;
      vector[n_tests] Fp_baseline;
      ////
      ...
      if (n_class == 2) {
          for (t in 1:n_tests) {
              Se_baseline[t] =       Phi( Xbeta_baseline_d[t] );
              Fp_baseline[t] =       Phi( Xbeta_baseline_nd[t] );
              Sp_baseline[t] = 1.0 - Fp_baseline[t];
          }
      }
      // ////
      // //// ---- likelihood:
