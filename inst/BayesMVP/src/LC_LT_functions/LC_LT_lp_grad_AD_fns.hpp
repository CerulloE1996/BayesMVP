...
void                             fn_lp_and_grad_LC_LT_AD_log_scale_InPlace_process(    Eigen::Ref<Eigen::Matrix<double, -1, 1>> out_mat,
  ...
  //// Phi_type and inv_Phi_type are validated here (unknown strings throw; previously an unknown Phi_type
  //// matched neither likelihood branch below, so the likelihood was silently left out) and honoured independently, as the latent_trait
  //// manual path does (fn_MVP_compute_lp_GHK_cols reads Phi_type and inv_Phi_type separately). See fn_AD_Phi_setting_from_strings.
  const AD_Phi_setting_struct AD_Phi_setting = fn_AD_Phi_setting_from_strings(  Phi_type,
                                                                              inv_Phi_type,
                                                                              "fn_lp_and_grad_LC_LT_AD_log_scale_InPlace_process");
  ...
  const Eigen::Matrix<double, -1, -1> LT_b_priors_1   = Model_args_as_cpp_struct.Model_args_mats_double[0]; 
  const Eigen::Matrix<double, -1, -1> LT_b_priors_2   = Model_args_as_cpp_struct.Model_args_mats_double[1]; 
  ...
  Eigen::Matrix<stan::math::var, -1, 1> u_prev_raw_var(n_pops); //// ----
   if (n_class > 1) { //// ----
     for (int g = 0; g < n_pops; ++g) {
       u_prev_raw_var(g) = theta_var(n_us + n_bs_LT + n_coeffs + g);
     }
   }
  ...
  if (nuisance_transformation == "Phi") { // correct
    ...
    //// Same transform and log-Jacobian as the latent_trait manual path (fn_MVP_compute_nuisance / _log_jac_u, MVP_manual_trans_and_J_fns.hpp):
    //// u = inv_logit(x), du/dx = u (1 - u).
    u_vec.array() =   inv_logit(u_unconstrained_vec_var).array();
    log_jac_u   +=    sum(log(u_vec));
    log_jac_u   +=    sum(log1m(u_vec));
  } else {
    throw std::invalid_argument("fn_lp_and_grad_LC_LT_AD_log_scale_InPlace_process: unknown nuisance_transformation '" + nuisance_transformation +
