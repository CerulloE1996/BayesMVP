...
inline void fn_lp_grad_MVOP_LC_Pinkney_NoLog_MD_and_AD_Inplace_process_serial(  Eigen::Ref<Eigen::Matrix<double, -1, 1>> out_mat ,
      ...
      const double log_det_J_main = log_det_J_prev_double_total + log_det_J_L_Omega_double + log_det_J_C_raw_to_C_double;
      ////
        ...
        for (int c = 0; c < n_class; c++) {
          ...
          for (int t = 0; t < n_tests; t++) {
            ...
            if (n_covariates_max > 1) {
                  ...
                  // ////
                  // for (int n = 0; n < N; n++ ) {
                  //   Xbeta_n = ( X[c][t].row(n).head(n_covariates_per_outcome_vec(c, t)).cast<double>() * beta_double_array[c].col(t).head(n_covariates_per_outcome_vec(c, t))  ).eval()(0, 0) ;
                  // } else {
                  //   Xbeta_n = beta_double_array[c](0, t);
                  // }
    ...
    if ((n_full_chunks < n_total_chunks) && (last_chunk_size > 0)) {
      ...
      ///////////////////////////////////////////////
      ws.dphi_times_bz.resize(last_chunk_size, n_tests); //// ordinal-only
      ws.dZ_times_bz.resize(last_chunk_size, n_tests); //// ordinal-only
      ///////////////////////////////////////////////
