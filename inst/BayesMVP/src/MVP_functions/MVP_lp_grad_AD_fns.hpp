...
inline  void                             fn_lp_and_grad_MVP_Pinkney_AD_log_scale_InPlace_process(    Eigen::Ref<Eigen::Matrix<double, -1, 1>> out_mat,
        ...
        //// as the manual paths do (see fn_AD_Phi_setting_from_strings in MVP_manual_trans_and_J_fns.hpp).
        const AD_Phi_setting_struct AD_Phi_setting = fn_AD_Phi_setting_from_strings(  Phi_type,
                                                                                    inv_Phi_type,
                                                                                    "fn_lp_and_grad_MVP_Pinkney_AD_log_scale_InPlace_process");
        ...
        //// Also release this thread's nested tape when a fallback evaluation throws.
        ...
        if (nuisance_transformation == "Phi") { // correct
          ...
          log_jac_u  +=   u_unconstrained_vec_var.size() * log(2.0) ;   //// du/dx = 2 u (1 - u), so +log(2) per coordinate (was -log(2) previously)   // correct
          log_jac_u   +=    sum(log(u_vec));  // correct
          log_jac_u   +=    sum(log1m(u_vec));  // correct
          ...
          throw std::invalid_argument("fn_lp_and_grad_MVP_Pinkney_AD_log_scale_InPlace_process: unknown nuisance_transformation '" + nuisance_transformation +
        ...
        for (int c = 0; c < n_class; ++c) {
              ...
              // Pinkney_cvine_bounds_opt 
              // Pinkney_LDL_bounds_opt
              ...
              // log_det_J_Omega(c) = Chol_Schur_outs(0, 1);
        ...
        // //   target_AD += stan::math::normal_lpdf(Omega_unconstrained_vec_var[i], 0.0, 10.0); 
        // // }
        // {
        ...
        for (int c = 0; c < n_class; c++) {
          ...
          // target_AD += (lkj_cholesky_eta(c) - 1.0) * log_det_J_Omega(c);  // (0, 1) holds log_det_C
        }
        /////////////////////////////////////////////////////////////////////////////////////////////////////////// likelihood
        ...
        if (AD_Phi_setting.use_Phi_approx == false) {   //// exact CDF (Phi_type = "Phi"); the inverse follows inv_Phi_type
          for (int n = 0; n < N; n++ ) {
            ...
            for (int c = 0; c < n_class; ++c) {
              ...
              for (int t = 0; t < n_tests; t++) {
                ...
                if ( (Bound_Z > overflow_threshold) &&  (y_ref(n, t) == 1) )   {
                        ...
                        using namespace stan::math;
                        //// EXACT normal tail (was the Phi_approx tail for every Phi_type).
                        ...
                        stan::math::var  Bound_U_Phi_Bound_Z = 0.0;
                  
                        if  (y_ref(n, t) == 1) {
                          //// right-side seam fixed by reflection. 1 - Phi(B) = Phi(-B) and
                          ...
                          Bound_U_Phi_Bound_Z = stan::math::Phi( Bound_Z );
                        ...
                        // // Inside the n loop, c loop, t loop, in the normal else block,
                        // // AFTER Z_std_norm(t) is computed and BEFORE the "if (t < n_tests - 1)" line:
                        // if (g_debug_cutpoint_grads == true) {
                        //   if (c == 0 && n < 3) {
                        //       double BZ_val = Bound_Z.val();
                        //       double BUPBZ_val = Bound_U_Phi_Bound_Z.val();
                        //       double PhiZ_val = Phi_Z.val();
                        //       double Z_val = Z_std_norm(t).val();
                        //       double prob_val, y1_val;
                        //       if (y_ref(n, t) == 1) {
                        //         prob_val = (1.0 - BUPBZ_val);
                        //         y1_val = y1(t).val();
                        //       } else {
                        //         prob_val = BUPBZ_val;
                        //         y1_val = y1(t).val();
                        //       }
                        //       std::cout << "AD n=0 c=0 t=" << t 
                        //                 << " Bound_Z=" << BZ_val
                        //                 << " Bound_U_Phi_BZ=" << BUPBZ_val
                        //                 << " Phi_Z=" << PhiZ_val
                        //                 << " Z_std_norm=" << Z_val
                        //                 << " prob=" << prob_val
                        //                 << " y1_log_prob=" << y1_val
                        ...
                        //       
                        //       if (c == 0) {
                        //         AD_diag_Bound_Z_val(n, t) = Bound_Z.val();
                        //         AD_diag_BUPBZ_val(n, t) = Bound_U_Phi_Bound_Z.val();
                        //         AD_diag_Phi_Z_val(n, t) = Phi_Z.val();
                        //         AD_diag_Z_val(n, t) = Z_std_norm(t).val();
                        //         if (y_ref(n, t) == 1) {
                        //           AD_diag_prob_val(n, t) = 1.0 - Bound_U_Phi_Bound_Z.val();
                        //         } else {
                        //           AD_diag_prob_val(n, t) = Bound_U_Phi_Bound_Z.val();
                        //         }
                        //       }
                        //       
                        //     }
                        // } // end of "g_debug_cutpoint_grads == true"
            ...
            if (n_class > 1) { //// if latent class 
              ...
              target_AD += log_posterior;
              //// Fill the observation outputs too; a fallback must not retain the previous position's log likelihoods.
          ...
          // if (g_debug_cutpoint_grads == true) {
          //   double sqrt_2_pi_recip_d = 1.0 / std::sqrt(2.0 * M_PI);
          //   for (int n = 0; n < std::min(3, N); ++n) {
          //     for (int t = 0; t < n_tests; ++t) {
          //       double BZ = AD_diag_Bound_Z_val(n, t);
          //       double BUPBZ = AD_diag_BUPBZ_val(n, t);
          //       double Z = AD_diag_Z_val(n, t);
          //       double pr = AD_diag_prob_val(n, t);
          //       
          //       double phi_BZ = sqrt_2_pi_recip_d * std::exp(-0.5 * BZ * BZ);
          //       double phi_Z = sqrt_2_pi_recip_d * std::exp(-0.5 * Z * Z);
          //       double phi_Z_rec = 1.0 / phi_Z;
          //       
          //       double y_val = y_ref(n, t);
          //       double y_sgn = 2.0 * y_val - 1.0;
          //       double u_val = u_array(n, t).val();
          //       double y_m_ys_xu = y_val - y_sgn * u_val;
          //       
          //       double L_tt_recip = 1.0 / L_Omega_var[0](t, t).val();
          //       
          //       // double dphi_over_L = y_sgn * phi_BZ * L_tt_recip;
          //       // double dZ_dmu_neg = y_m_ys_xu * phi_Z_rec * phi_BZ * L_tt_recip;
          //       
          //       double y_sign_times_phi_BZ_x_L_recip = y_sgn * phi_BZ * L_tt_recip;
          //       double y_m_ysign_xu_times_phiZ_phiBZ_L_recip = y_m_ys_xu * phi_Z_rec * phi_BZ * L_tt_recip;
          //       
          //       std::cout << "AD c=0 n=" << n << " t=" << t
          //                 << " phi_Z_recip=" << phi_Z_rec
          //                 << " phi_Bound_Z=" << phi_BZ
          //                 << " y_sign=" << y_sgn
          //                 << " y_m_ysign_xu=" << y_m_ys_xu
          //                 // << " dphi_over_L=" << dphi_over_L
          //                 // << " dZ_dmu_neg=" << dZ_dmu_neg
          //                 << " ys_phiBZ_Lrecip=" << y_sign_times_phi_BZ_x_L_recip
          //                 << " ymysxu_phiZr_phiBZ_Lr=" << y_m_ysign_xu_times_phiZ_phiBZ_L_recip
          //                 << " prob=" << pr
          //                 << " prob_recip=" << 1.0 / pr
          //                 << std::endl;
          //     }
        ...
        } else {   //// Phi_approx CDF (Phi_type = "Phi_approx" / "Phi_approx_2", validated above); the inverse follows inv_Phi_type
          ...
          for (int n = 0; n < N; n++ ) {
            ...
            for (int c = 0; c < n_class; ++c) {
              ...
              for (int t = 0; t < n_tests; t++) {
                    ...
                    if ( (Bound_Z > overflow_threshold) &&  (y_ref(n, t) == 1) )   {
                            ...
                            Z_std_norm(t) =  fn_AD_inv_Phi_from_log_probs_var(log_Phi_Z, log_1m_Phi_Z, use_inv_Phi_approx);   //// inv_Phi_approx from logit(Phi_Z) unless inv_Phi_type = "inv_Phi"
                            ...
                            if  (y_ref(n, t) == 1) {
                              ...
                              Z_std_norm(t) =   fn_AD_inv_Phi_var(Phi_Z, use_inv_Phi_approx) ;   //// inverse follows inv_Phi_type
                            } else {
                              stan::math::var  Bound_U_Phi_Bound_Z = stan::math::Phi_approx( Bound_Z );
                              y1(t) = stan::math::log(Bound_U_Phi_Bound_Z);
                              Phi_Z  =  Bound_U_Phi_Bound_Z * u_array(n, t);
            ...
            if (n_class > 1) { //// if latent class 
              stan::math::var log_posterior = stan::math::log_sum_exp(lp);
              target_AD += log_posterior;
              //// Fill the observation outputs too, including when Phi_approx is selected.
              if (out_mat.size() >= 1 + n_params + N) out_mat(1 + n_params + n) = log_posterior.val();
            } else {  /// if NOT latent class
              stan::math::var log_posterior = lp(0);
              target_AD += log_posterior;
        ...
        //// nested_autodiff_scope releases the tape on both normal return and exceptions.
...
inline void     fn_lp_and_grad_MVP_Pinkney_AD_log_scale_InPlace(   Eigen::Matrix<double, -1, 1> &&out_mat_R_val, 
...
// Internal function using Eigen::Ref as inputs for matrices
inline void     fn_lp_and_grad_MVP_Pinkney_AD_log_scale_InPlace(   Eigen::Matrix<double, -1, 1> &out_mat_ref, 
                                                                   const Eigen::Matrix<double, -1, 1> &theta_main_vec_ref,
                                                                   const Eigen::Matrix<double, -1, 1> &theta_us_vec_ref,
                                                                   const Eigen::Matrix<int, -1, -1> &y_ref,
                                                                   const std::string &grad_option,
...
inline void     fn_lp_and_grad_MVP_Pinkney_AD_log_scale_InPlace(   Eigen::Ref<Eigen::Block<MatrixType, -1, 1>>  &out_mat_ref, 
...
// Internal function using Eigen::Ref as inputs for matrices
inline Eigen::Matrix<double, -1, 1>    fn_lp_and_grad_MVP_Pinkney_AD_log_scale(  const Eigen::Ref<const Eigen::Matrix<double, -1, 1>> theta_main_vec_ref,
