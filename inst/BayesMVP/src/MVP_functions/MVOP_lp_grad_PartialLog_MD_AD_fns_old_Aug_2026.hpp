...
inline void fn_lp_grad_MVOP_LC_Pinkney_PartialLog_process_chunk( Eigen::Ref<Eigen::Matrix<double, -1, 1>> out_mat,
                                                                        ...
                                                                        //// accumulators (accumulated ACROSS chunks - do not reset here):
                                                                        double &log_jac_u,
                                                                        ...
                                                                        //// SIMD struct for full chunks / "Stan" (scalar) copy for the remainder chunk:
        ...
        //// TRUE => scalar Stan fns; FALSE => custom fast/SIMD fns. Must match the
        //// standard-scale pass that produced this chunk's inputs (the remainder chunk
        //// gets the "Stan" struct copy, so this flips automatically):
        const bool S = (Model_args_for_chunk.Model_args_strings(0) == "Stan");
        ...
        //////////////////////////////////////////////////////////////////////////////////////////
        //// ---- Local containers (allocated at chunk_size - no ws struct, like MVP PartialLog):
        //////////////////////////////////////////////////////////////////////////////////////////
        //// per-class containers (NOTE: y1_log_prob is PER-CLASS here - unlike NoLog, which re-uses
        //// one matrix across classes - because the per-row log-scale grad engine needs the exact
        //// per-class log-probs of the problem rows at grad time):
        ...
        std::vector<Eigen::Matrix<double, -1, -1>>   Upper_Bound_Z = Z_std_norm; //// ordinal-only
        ...
        //// per-class log containers - REQUIRED OUTPUTS of the MVP binary log-scale lp handlers
        //// (the per-row grad engine itself recomputes log-phi terms analytically, so these are
        //// not consumed downstream):
        std::vector<Eigen::Matrix<double, -1, -1>>   log_Z_std_norm  = vec_of_mats<double>(chunk_size, n_tests, n_class);
        std::vector<Eigen::Matrix<double, -1, -1>>   log_phi_Bound_Z = log_Z_std_norm;
        std::vector<Eigen::Matrix<double, -1, -1>>   log_phi_Z_recip = log_Z_std_norm;
        ///////////////////////////////////////////////
        ...
        Eigen::Matrix<double, -1, -1> phi_Upper_Bound_Z = Eigen::Matrix<double, -1, -1>::Zero(chunk_size, n_tests); //// ordinal-only
        ///////////////////////////////////////////////
        ...
        ///////////////////////////////////////////////
        Eigen::Matrix<double, -1, -1> dphi_over_L =   Eigen::Matrix<double, -1, -1>::Zero(chunk_size, n_tests); //// ordinal-only fn
        Eigen::Matrix<double, -1, -1> dZ_dmu_neg =    Eigen::Matrix<double, -1, -1>::Zero(chunk_size, n_tests); //// ordinal-only fn
        Eigen::Matrix<double, -1, -1> dphi_times_bz = Eigen::Matrix<double, -1, -1>::Zero(chunk_size, n_tests); //// ordinal-only fn
        Eigen::Matrix<double, -1, -1> dZ_times_bz =   Eigen::Matrix<double, -1, -1>::Zero(chunk_size, n_tests); //// ordinal-only fn
        ///////////////////////////////////////////////
        ...
        Eigen::Matrix<double, -1, 1> dphi_direct_Cj = Eigen::Matrix<double, -1, 1>::Zero(chunk_size); //// ordinal-only
        Eigen::Matrix<double, -1, 1> dZ_direct_Cj =   Eigen::Matrix<double, -1, 1>::Zero(chunk_size); //// ordinal-only
        ///////////////////////////////////////////////
        ...
        ///////////////////////////////////////////////
        //// problem bookkeeping: per-class row flags (union over tests + global log-lik underflow rows):
        ...
        for (int c = 0; c < n_class; c++) {
          ...
          for (int t = 0; t < n_tests; t++) {
            ...
            if (n_covariates_max > 1) {
                  ...
                  if (t_ord_slot < 0) { // binary
                  ...
                  } else { // Store mu_t temporarily, compute bounds per-observation
                        ...
                        for (int n = 0; n < chunk_size; ++n) {
                              ...
                              Upper_Bound_Z[c](n, t) = ub;     // upper_bz
            ...
            ////-----------------------------------------------
            //// ---- masks / problem-index detection (PartialLog) ----
            //// binary: same masks as the MVP PartialLog. ordinal: BOTH bounds deep in the SAME tail
            //// (lb > overflow_threshold -> both upper tail; ub < underflow_threshold -> both lower
            //// tail), which is exactly when prob = Phi(ub) - Phi(lb) underflows / catastrophically
            //// cancels. (Bounds straddling zero can never make prob tiny.)
            std::vector<int> over_index;
            std::vector<int> under_index;
            std::vector<int> ord_problem_index;
            
            // if (t < n_binary_tests) {
            if (ord_idx_of_test(t) < 0) { // if binary
              
                for (int n = 0; n < chunk_size; ++n) {
                  const double BZ_n = Bound_Z[c](n, t);
                  const double y_n_dbl = y_chunk(n, t);
                  if        ( (BZ_n > overflow_threshold)  && (y_n_dbl == 1.0) ) {
                    over_index.push_back(n);
                    row_is_problem[c][n] = 1;
                  } else if ( (BZ_n < underflow_threshold) && (y_n_dbl == 0.0) ) {
                    under_index.push_back(n);
                    row_is_problem[c][n] = 1;
                  }
                }
                ...
                for (int n = 0; n < chunk_size; ++n) {
                  const double lb_n = Bound_Z[c](n, t);
                  const double ub_n = Upper_Bound_Z[c](n, t);
                  if ( (lb_n > overflow_threshold) || (ub_n < underflow_threshold) ) {
                    ord_problem_index.push_back(n);
                    row_is_problem[c][n] = 1;
                  }
                }
            ...
            ////-----------------------------------------------
            //// compute/update important log-lik quantities for GHK-MVP (standard scale, ALL rows):
            // if (t < n_binary_tests) {
            if (ord_idx_of_test(t) < 0) { // if binary
                  ...
                  fn_MVP_compute_lp_GHK_cols(  t,
                                               ...
                                               y1_log_prob[c],  // computing this (PER-CLASS here)
                                               Bound_Z[c],
                                               y_chunk,
                                               u_array,
                  ...
                  fn_MVOP_compute_lp_GHK_cols( t,
                                               ...
                                               u_array, 
                                               ////
                                               Upper_Bound_Z[c], // ordinal-only - upper_bz
                                               ////
                                               // n_binary_tests, // ordinal-only
                                               false,  // is_binary (this branch is ordinal)
                                               Model_args_for_chunk);
              
            }
            ////-----------------------------------------------
            //// ---- log-scale fix-up of the problem entries (EXACT, overwrites std-scale garbage): ----
            // if (t < n_binary_tests) {
            if (ord_idx_of_test(t) < 0) { // if binary
              
                  if (under_index.size() > 0) { //// underflow (w/ y == 0)
                  ...
                  if (over_index.size() > 0) { //// overflow (w/ y == 1)
                        const int n_overflows = over_index.size();
                  ...
                  if (ord_problem_index.size() > 0) {
                    fn_MVOP_compute_lp_GHK_cols_log_scale_ordinal( t,
                                                                   ord_problem_index,
                                                                   Bound_U_Phi_Bound_Z[c], // computing this
                                                                   Phi_Z[c],  // computing this
                                                                   Z_std_norm[c],  // computing this
        ...
        log_prob_n_recip = -log_lik_chunk;  //// EXACT log(1 / prob_n) - never overflows (used by the per-row log-scale engine)
        ////
        //// rows whose TOTAL likelihood underflowed: prob_n_recip = 1/exp(log_lik) = Inf. Zero it
        //// (so the standard-scale prev / grad-prep terms for these rows are 0 rather than Inf/NaN)
        //// and mark the row as a problem row in ALL classes -> handled by the log-scale engine:
        for (int n = 0; n < chunk_size; ++n) {
          if (log_lik_chunk(n) < -690.0) {
            prob_n_recip(n) = 0.0;
            for (int c2 = 0; c2 < n_class; ++c2)  row_is_problem[c2][n] = 1;
          }
        }
        ////-----------------------------------------------
        ...
        for (int c = 0; c < n_class; c++) {
          ...
          //// ---- re-fill prev (+ LOG prev - needed by the log-scale engine) for this class ----
          if (n_class > 1) {
            for (int n = 0; n < chunk_size; ++n) {
              int n_global = chunk_size_orig * chunk_counter + n;
              int g = pop_ind(n_global);
          ...
          ////-----------------------------------------------
          prob_recip = stan::math::inv(prob[c]);   //// Inf at problem entries - those ROWS are zeroed below
          ////-----------------------------------------------
          ...
          ////-----------------------------------------------
          //// ---- per-class problem-row list (built ONCE per class, used for zeroing + engine): ----
          std::vector<int> P_c;
          if (grad_option != "none") {
          ...
          if (grad_option != "none") {
                ...
                fn_MVOP_grad_prep( prob[c],
                                   ...
                                   ////
                                   dphi_times_bz, //// for ordinal-only fn
                                   dZ_times_bz, //// for ordinal-only fn
                                   ////
                                   // n_binary_tests, //// for ordinal-only fn
                                   ord_idx_of_test, //// slot-type map
                                   ////
                                   Model_args_for_chunk);
                
                //// ---- ZERO the problem rows' standard-scale gradient inputs. ----
                //// After this, EVERY standard-scale gradient function below contributes exactly 0
                //// for the problem rows (all their per-row factors are 0 - no 0*Inf = NaN left).
                //// The EXACT contributions of these rows are then ADDED by the per-row
                //// signed-log engine (fn_MVOP_row_grads_log_scale) further below.
                for (size_t pi = 0; pi < P_c.size(); ++pi) {
                  
                      const int n = P_c[pi];
                      common_grad_term_1.row(n).setZero();
                      prob_recip.row(n).setZero();
                      prob_rowwise_prod_temp.row(n).setZero();
                      prob_recip_rowwise_prod_temp.row(n).setZero();
                      prob_rowwise_prod_temp_all(n) = 0.0;
                      dphi_over_L.row(n).setZero();
                      dZ_dmu_neg.row(n).setZero();
                      dphi_times_bz.row(n).setZero();
                      dZ_times_bz.row(n).setZero();
                      phi_Z_recip.row(n).setZero();
                      prob[c].row(n).setZero();  //// kills the std-scale prev contribution of these rows too
          ...
          if (do_us) {
                ...
                //// NOTE: unlike NoLog, the out_mat u-grad copy + Jacobian adjustment is moved to the
                //// END of this c loop body (after the log-scale engine has added the problem rows'
                //// exact u-grad contributions for this class).
          ...
          if (do_cut) {
                ...
                for (int t = 0; t < n_tests; ++t) {
                  ...
                  fn_MVOP_compute_cutpoint_grad(     c, 
                                                     cutpoint_grad_array[c],  // (n_cutpoints_max x n_ordinal_tests) 
                                                     t, 
                                                     t_ord, 
                                                     K_t,
                                                     common_grad_term_1,
          ...
          ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
          //// ---- LOG-SCALE PER-ROW ENGINE: add the EXACT gradient contributions of the problem  ----
          //// ---- rows for this class (their standard-scale contributions above were exactly 0). ----
          if ( (grad_option != "none") && (P_c.size() > 0) && (do_us || do_coeff || do_corr || do_cut) ) {
            
                for (size_t pi = 0; pi < P_c.size(); ++pi) {
                  
                      const int n = P_c[pi];
                      const int n_global = chunk_size_orig * chunk_counter + n;
                      const double log_prev_nc = (n_class > 1) ? log_prev_per_obs_given_c(n) : 0.0;
                      
                      fn_MVOP_row_grads_log_scale(  n, n_global, c,
                                                    do_us, do_coeff, do_corr, do_cut,
                                                    n_tests, ord_idx_of_test, n_class, n_covariates_max,
                                                    n_cov_vec_c,
                                                    n_cat_per_ord_test,
                                                    X[c],
                                                    y_chunk,
                                                    ...
                                                    u_grad_array_CM_chunk,
                                                    beta_grad_array[c],
                                                    U_Omega_grad_array[c],
                                                    cutpoint_grad_array[c],
                                                    S);
          ...
          if (n_class > 1) { /// prevelance only estimated for latent class models
                ...
                if (do_prev) {
                      ...
                      //// standard-scale accumulation: problem rows contribute EXACTLY 0 here, because
                      //// prob[c].row(n) was zeroed for n in P_c (so the rowwise product is 0), and
                      //// prob_n_recip was zeroed for the global log-lik-underflow rows:
                      ...
                      //// log-scale engine for the problem rows:
                      ////   prev_grad_mat(g, c) += prod_t prob_c(n, t) / prob_n(n)
                      ////                        = exp( sum_t y1_log_prob_c(n, t) + log_prob_n_recip(n) )
                      for (size_t pi = 0; pi < P_c.size(); ++pi) {
                        const int n = P_c[pi];
                        const int n_global = chunk_size_orig * chunk_counter + n;
                        const int g = pop_ind(n_global);
                        prev_grad_mat(g, c) += std::exp( y1_log_prob[c].row(n).sum() + log_prob_n_recip(n) );
                      }
          ...
          ///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
          //// ---- u-grad out_mat copy + Jacobian adjustments (moved from the nuisance section; ----
          //// ---- must run AFTER the log-scale engine's u-grad additions for the final class): ----
...
inline void fn_lp_grad_MVOP_LC_Pinkney_PartialLog_MD_and_AD_Inplace_process_serial(  Eigen::Ref<Eigen::Matrix<double, -1, 1>> out_mat ,
                                                                                     ...
                                                                                     std::vector<LC_MVP_workspace_struct> &LC_MVP_ws_structs //// UNUSED in PartialLog (local containers) - kept for interface parity
) {
        ...
        ////
        // ============================================================================
        // CHUNK LOOP - PartialLog version.
        // ONE shared per-chunk routine (lp + grad + log-scale fix-ups). Full chunks
        // receive the SIMD-configured struct; the remainder chunk receives a "Stan"
        // (scalar) COPY of the struct, so it is genuinely scalar end-to-end (this is
        // the same remainder fix applied to the NoLog files).
        // ============================================================================
        for (int nc = 0; nc < n_full_chunks; nc++) {
          
              fn_lp_grad_MVOP_LC_Pinkney_PartialLog_process_chunk(   out_mat,
                                                                     theta_us_vec_ref,
                                                                     y_ref,
                                                                     grad_option,
                                                                     nc,                  //// chunk_counter
                                                                     normal_chunk_size,   //// chunk_size (all full chunks are the same size)
                                                                     chunk_size_orig,
                                                                     N, n_params,
                                                                     n_tests, n_class, n_binary_tests, n_ordinal_tests, ord_idx_of_test,
                                                                     n_covariates_max, n_pops,
                                                                     overflow_threshold, underflow_threshold,
                                                                     ...
                                                                     prev_grad_mat,
                                                                     ////
                                                                     Model_args_as_cpp_struct);   //// full chunks: SIMD struct as-is
        ...
        if ((n_full_chunks < n_total_chunks) && (last_chunk_size > 0)) {
              ...
              //// "Stan" (scalar) copy of the struct - ALL helper fns receive this for the remainder:
              ...
              fn_lp_grad_MVOP_LC_Pinkney_PartialLog_process_chunk(   out_mat,
                                                                     ...
                                                                     prev_grad_mat,
                                                                     ////
                                                                     Model_args_last_chunk);   //// remainder: scalar struct
